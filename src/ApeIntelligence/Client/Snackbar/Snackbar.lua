...
local SnackbarStatic = {}
SnackbarStatic.QueChange = Signal.new()
SnackbarStatic.PlayingQue = false
SnackbarStatic.Que = {}
...
SnackBar.Position = UDim2.new(1, -20, 1, 10 - SnackBar.Height)
...
local function RenderQue()
	if SnackbarStatic.PlayingQue then return end
	SnackbarStatic.PlayingQue = true

	local _, Snackbars = next(SnackbarStatic.Que)
	while Snackbars do
		Snackbars.Gui.Parent = SnackTray
		Snackbars.Tween:Play()

		Snackbars.Tween.Completed:Wait()
		SnackbarStatic.QueChange:Fire(Snackbars)

		_, Snackbars = next(SnackbarStatic.Que)
	end
	SnackbarStatic.PlayingQue = false
end
...
function SnackBar:CreateSnackbar(Text, Theme, Timeout, TextStyle)
	...
	local Pos = self.Position + UDim2.new(0, (-Width) + (Width/2), 0, 0)
	...
	table.insert(SnackbarStatic.Que, self)
...
function FadeSnackbar(SnackbarGui, Speed, Properties)
	...
	-- only children of SnackBarUI should be TextLabel
	local BaseProperties = {
		Size = UDim2.new(0, 0, 0, 0);
		BackgroundTransparency = 1;
		ImageTransparency = 1
	}
	local Tween = TweenService:Create(SnackbarGui, Info, BaseProperties)

	local PropertiesTable = Properties or {
		["TextLabel"] = {TextTransparency = 1; TextStrokeTransparency = 1;};
	}

	for _, GuiBase in pairs(SnackbarGui:GetDescendants()) do
...
SnackbarStatic.QueChange:Connect(function(Snackbar)
	local Find = table.find(SnackbarStatic.Que, Snackbar)

	if Find then
		table.remove(SnackbarStatic.Que, Find)
		FadeSnackbar(Snackbar.Gui, .2)
		Snackbar.Gui:Destroy()
	end
end)
